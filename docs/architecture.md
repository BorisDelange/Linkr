...
- **Database cohort boards**: `PatientDashboard` has the same one-owner rule; a database's boards are its cohorts' — **one per cohort** (`ownerDataSourceId` for access and cleanup + `ownerCohortId`, cascading with the cohort; the server refuses a second, 409, and a cohort of another database, 422), created lazily (`ensureCohortBoard`) and shown in the cohort's *Patients* results tab (`CohortPatientsPanel`: the last execution's rows as the patient list, the board beside it). Store maps are keyed `cohortBoardKey(id)` = `cohort:<id>` in place of a project uid; `PatientChartContext.can` / `codeWidgets` carry the workspace role and the fact that server-mode R/Python needs a project session (so only SQL widgets are offered there). Exported as `databases/<eid>/cohort-boards/<cohort key>.json` beside `cohorts/<cohort key>.json`, the shape of a project's `patient-dashboards/*.json` (shared `patientBoardBundle` / `_build_patient_dashboard_json`); replaced whole on import/pull after the cohorts (`replaceDatabaseBoards`), ids derived from the database, the cohort key and the content keys.
...
- **Jobs**: long-running work is a DB-backed `jobs` row (`kind` build|run|derive, `status` queued|running|done|error|cancelled, `progress`, `log_tail`, `result`), run behind a bounded in-process executor (`asyncio.Semaphore(max_build_concurrency)`, default 2 — no external broker). A job has exactly one owner: `project_uid` (env builds, batch runs) or `workspace_id` (a database's cohort derivation, `POST /data-sources/{id}/derive` → 202 + the queued job). `/projects/{uid}/jobs`, `/workspaces/{id}/jobs` + `/jobs/{id}/cancel`; orphaned `running` jobs are reconciled to `error` at startup. Footer `JobsIndicator` polls the open project's and the open workspace's jobs, cancels, and re-reads what a job changed when it ends (`derivationFinished` for a derivation). A derivation reports progress per table and is cancelled by interrupting the statement in flight (`DeriveControl`); what it half-wrote is removed, and a new database created for a first build that fails is deleted.
...
- **Dataset edit log**: a dataset is editable without its raw file ever being written to. Edits are an ordered, replayable log of operations (`setCell`, `addRow`, `removeRow`, `reorderRows`, `addColumn`, `removeColumn`, `reorderColumns`, `renameColumn`) held in the `ops` section of the same sidecar, and the materialised form is derived: `raw → parse(parseOptions) → replay(ops) → parquet`. The cache is therefore keyed on the raw `(mtime, size)` **and** the ops digest. Ops are keyed by **raw row ordinal**, not position — the table shows a sorted/filtered/paginated slice and server mode holds one page, so a positional op is not replayable; rows added by an op take negative ordinals, disjoint from raw ones by construction. `POST /dataset-files/ops` **appends** (concatenating server-side, idempotent per op id) so two people collecting on different patients cannot drop each other's work; `replace` is kept for compaction and reset-to-raw. The engine is a TS/Python twin pair (`packages/linkr-format/src/dataset-ops.ts`, `app/services/data/dataset_ops.py`) under a shared fixture covering replay, the canonical wire form and its digest — the client replays to render, the server to build the cache, and a drift would make the two disagree silently. Undo appends inverses (grouped, so one user action reverses whole) and so survives a reload, unlike the closure-based `UndoAction` stack. A rename is a **rekey** — ids derive from names — so it repairs dashboard filters and widget configs in the same change (matched by value, never by key name) and refuses a name whose slug would displace another column. The log travels in its OWN file beside the data it applies to — `datasets/<dataset>/<name>.edits.json` — written and gitignored on the same "mark for versioning" as `<name>.csv`. It used to ride inline in `datasets/_tree.json`, which is wrong on privacy grounds: one tree describes every dataset in the project, so it can never be excluded for one of them, and a `setCell` carries its value — for a dataset filled by hand (a manual collection) the journal IS the health data, since the raw file is immutable and may be empty or absent. Gitignoring the CSV therefore did not stop the values leaving. `_tree.json` now keeps structure only (columns, labels, constraints, `parseOptions`); `rowCount` went with the journal, being a count of records. Older trees carrying `ops` inline are still read, so an existing export re-imports with its edits intact. Two consequences: an *edited* `.parquet` stops being its own cache (replay would otherwise have to write the raw), and the cache carries the row key as a non-column only while a log exists. Plan: `docs/planning/dataset-edit-plan.md`.
- **Datasets**: raw files on disk are immutable; server-paginated preview/rows (`LIMIT/OFFSET`, `ORDER BY`/`WHERE`) + per-column stats as DuckDB aggregates; `datasets/` surfaced read-only in the IDE tree. A per-dataset sidecar `projects/<uid>/dataset-meta/<hash>.json` carries column metadata (label/description/valueLabels) + `parseOptions` (columnTypes/filterMode/delimiter), merged in `dataset_fs.resolve_cache` and travelling on export/git. A column's `label` and `description` are **LocalizedString** (`{en, fr}`), like every other authored text: read them with `displayColumnName`/`displayColumnDescription`, never `col.label` directly, and write them with `cleanLocalized` so an emptied one drops out instead of storing blank languages. A legacy plain string still reads everywhere (the validator warns rather than errors).
- **Server file browser & `serverPath` databases**: `services/fs_browser.py` lists the server filesystem for two callers — a project binding its IDE/datasets folders, and a **database pointing at data already on the server** instead of uploading it (a warehouse past the 2 GB `max_upload_mb` ceiling has no other route). One shared dialog, `ServerPathPickerDialog` (`mode` folder|file, `scope` project|workspace); files are listed in both modes, disabled in folder mode so a folder is recognisable by what it holds. Routes are split by authority: `/projects/{uid}/fs/*` needs `project-settings:write`, `/workspaces/{id}/fs/*` needs `databases:write` — browsing always answers to an explicit permission. **Confinement is `settings.fs_browse_roots`** (CSV of absolute paths, empty = the whole filesystem — the RStudio Server model, where the deployment mounts only what the server should reach); paths are `resolve()`d so symlinks cannot escape a root. `serverPath` rides in `connection_config` beside the `managed` flag (no migration) and resolves in `data_source_service._source_files()`: a file attaches as a file, a directory contributes its Parquet recursively — and unlike the blob store, a real folder keeps names and extensions, so `connection-info` can hand a client library a path it can glob. Two invariants: the boundary is **re-enforced where the config is persisted** (`enforce_server_path` in create *and* update — the picker's check is a convenience, a hand-made PATCH is not), and a `serverPath` source is never `managed`, so `run_etl_sql` refuses it as a target and Linkr never writes a file it does not own. The converse — a file Linkr **creates** outside its data folder — is `managed` + `managedPath` (Create from schema, *Location*): set only by `create-from-ddl`, once, after `check_new_database_file` (a new `*.duckdb`, in a writable folder inside the roots, **never an existing file** — Linkr writes into it and a rebuild deletes it); `create`/`update` drop any `managedPath` a client sends and keep the stored one (`_keep_managed_path`), so it can neither be set nor moved by a plain PATCH. Every managed-file operation resolves through `data_source_service.managed_path()`; deleting the database leaves a `managedPath` file in place. Deliberately **not** gated on `enable_code_execution` (unlike `validate_binding_path`): attaching a database read-only is not code execution, so a deployment with the IDE off can still point Linkr at its own data. The path is machine-local like the folder bindings — `EXPORTED_CONNECTION_KEYS` withholds it from exports by construction.
