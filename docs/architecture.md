...
- **Jobs**: long-running work is a DB-backed `jobs` row (`kind` build|run|derive, `status` queued|running|done|error|cancelled, `progress`, `log_tail`, `result`), run behind a bounded in-process executor (`asyncio.Semaphore(max_build_concurrency)`, default 2 — no external broker). A job has exactly one owner: `project_uid` (env builds, batch runs) or `workspace_id` (a database's cohort derivation, `POST /data-sources/{id}/derive` → 202 + the queued job). `/projects/{uid}/jobs`, `/workspaces/{id}/jobs`, `GET /jobs/{id}` (owner only — an agent following its job) + `/jobs/{id}/cancel`; orphaned `running` jobs are reconciled to `error` at startup. Footer `JobsIndicator` polls the open project's and the open workspace's jobs, cancels, and re-reads what a job changed when it ends (`derivationFinished` for a derivation). A derivation reports progress per table and is cancelled by interrupting the statement in flight (`DeriveControl`); what it half-wrote is removed, and a new database created for a first build that fails is deleted.
...
- **Concept mapping**: `tools-mapping.ts` reads a mapping project's flat source (`POST /mapping-projects/{id}/query`), searches the project's OMOP vocabulary database with the app's own builders (`mapping-queries.ts`, `concept-detail-queries.ts`; the first concept table that is an OMOP `concept` table, since a project without a vocabulary database falls back to its source database), and writes either AI suggestions — `POST /mapping-projects/{id}/scores/append` merges `ai/<model>` rows into the project's scores parquet server-side (DuckDB, key `(vocabulary, code, concept_id, method)`, existing rows kept, file created when absent) — or mappings through `/concept-mappings/batch`, then refreshes the project's `stats` as the app does. Both writes notify (`entity_type` `mapping_project`); the front refresher reloads the open project's mappings and re-indexes its scores.
...
- **Documentation**: `search_docs` / `read_doc` read the user docs from `/docs-index.json`, which linkr-website builds from its MDX pages (`src/lib/docs-index/`, along with `llms.txt` and `llms-full.txt`). The website stays the single source of the user docs; the MCP only downloads its published index, so any instance searches the docs matching the published site.
