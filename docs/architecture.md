...
- **Database patient board**: `PatientDashboard` has the same one-owner rule; a database owns **one** (server refuses a second, 409), created lazily (`ensureDatabaseBoard`) and shown in a database cohort's *Patients* results tab (`CohortPatientsPanel`: the last execution's rows as the patient list, the board beside it). Store maps are keyed `databaseBoardKey(id)` = `db:<id>` in place of a project uid; `PatientChartContext.can` / `codeWidgets` carry the workspace role and the fact that server-mode R/Python needs a project session (so only SQL widgets are offered there). Exported as `databases/<eid>/patient-board.json`, the shape of a project's `patient-dashboards/*.json` (shared `patientBoardBundle` / `_build_patient_dashboard_json`), replaced whole on import/pull (`replaceDatabaseBoard`).
