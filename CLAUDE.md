...
**Releasing** — when the user asks for a release `vX.Y.Z`: set `VERSION`, regenerate the 21 export goldens that pin it, and pin BOTH image lines in `docker/docker-compose.hub.yml` to `api-X.Y.Z` / `web-X.Y.Z` (that file is what users download, so a stale one points every new install at the previous version). Then tag. The tag pipeline re-checks all three and refuses to publish if any disagrees, so a miss fails the release rather than shipping. Images are published by CI on the tag — never push them from a dev machine, which would produce arm64 images for an amd64 audience.