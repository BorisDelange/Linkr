...
async def _load(db: AsyncSession, concept_set_id: str, user: User, min_role: str) -> ConceptSet:
    concept_set = await concept_set_service.get(db, concept_set_id)
    if concept_set is None:
        raise HTTPException(status.HTTP_404_NOT_FOUND, "Not found")
    await check_workspace_role(db, concept_set.workspace_id, user, min_role)
    return concept_set
...
async def list_concept_sets(
    ...
    for cs in sets:
        try:
            await check_workspace_role(db, cs.workspace_id, user, "viewer")
            visible.append(cs)
...
async def delete_batch(
    ...
    for cid in body.ids:
        ...
        if cs is None:
            continue
        await check_workspace_role(db, cs.workspace_id, user, "editor")
...
async def get_concept_set(
    ...
    return await _load(db, concept_set_id, user, "viewer")
...
async def update_concept_set(
    ...
    concept_set = await _load(db, concept_set_id, user, "editor")
