...
class ExecutionSession(Base, TimestampMixin):
    ...
    by (project, user, language, session). The interpreter + packages come from the
    project's managed environment (resolved separately, one per language); the
    session only selects which live process among a user's own — a session is a
    namespace, not a virtualenv.
    ...
    # Client-supplied UUID, used as the kernel's session_id.
