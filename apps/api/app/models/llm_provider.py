...
class LlmProvider(Base, UUIDPKMixin, TimestampMixin):
    ...
    # Surfaces this model is approved for ("dashboard", "ide", …), as decided by
    # an admin. A model can be good at driving a dashboard and poor in the IDE,
    # so approval is per surface rather than global. Empty list = configured but not offered anywhere yet.
