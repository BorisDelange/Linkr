"""Regression render: server-owned statsmodels program + spec validation.

The `_REG_PY` body is ported verbatim from the frontend regression-server.ts
(`_REG_PY`) — it must stay in parity with runRegression in RegressionComponent.tsx.
Only the spec (outcome + predictors + type + confidence) varies per request.
"""

import json

_ALLOWED_TYPES = {"auto", "linear", "logistic"}


def validate_spec(spec: dict) -> dict:
    """Coerce + validate the client spec into the shape _REG_PY expects:
    {outcome: {name: str, numeric: bool}, predictors: [{name: str, numeric: bool}],
    regressionType: str, confidenceLevel: float}.
    Raises ValueError on a malformed spec so the route returns a clean 400."""
    if not isinstance(spec, dict):
        raise ValueError("regression spec must be an object")
    raw_outcome = spec.get("outcome")
    if not isinstance(raw_outcome, dict) or not isinstance(raw_outcome.get("name"), str):
        raise ValueError("regression spec.outcome must be {name: str, numeric: bool}")
    outcome = {"name": raw_outcome["name"], "numeric": bool(raw_outcome.get("numeric"))}
    raw_predictors = spec.get("predictors") or []
    if not isinstance(raw_predictors, list):
        raise ValueError("regression spec.predictors must be a list")
    predictors = []
    for p in raw_predictors:
        if not isinstance(p, dict) or not isinstance(p.get("name"), str):
            raise ValueError("regression spec.predictors entries need a string name")
        predictors.append({"name": p["name"], "numeric": bool(p.get("numeric"))})
    reg_type = spec.get("regressionType", "auto")
    if reg_type not in _ALLOWED_TYPES:
        reg_type = "auto"
    try:
        # confidenceLevel is a PERCENTAGE (0–100): _REG_PY computes alpha = 1 - conf/100,
        # matching the component (config.confidenceLevel ?? 95). Default 95, not 0.95.
...
def build_code(spec: dict) -> str:
    ...
    return f"{_REG_PY}\n_linkr_print_reg(dataset, _json.loads({embedded}))\n"
