/**
 * Shared markdown readme editor with toolbar, split view (edit/preview), and Cmd+S save.
 * Used by both Project summary and Workspace home pages.
 */
...
import {
  ...
  Code, Quote, Link as LinkIcon, Image,
  Minus, Table, Sigma, HelpCircle, Copy,
} from 'lucide-react'
...
interface ReadmeEditorProps {
  ...
  /** Optional function to resolve attachment URLs in markdown (e.g. `attachments/foo.png` → blob:...) */
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <DialogTrigger asChild>
                <button type="button" className="flex h-7 w-7 items-center justify-center rounded-md text-muted-foreground hover:bg-accent hover:text-foreground">
                  <HelpCircle size={14} />
                </button>
              </DialogTrigger>
            </TooltipTrigger>
...
function CopyButton({ text }: { text: string }) {
  ...
  return (
    <button type="button" onClick={handleCopy} className="flex h-5 w-5 shrink-0 items-center justify-center rounded text-muted-foreground/50 transition-colors hover:bg-accent hover:text-foreground">
...
function MarkdownHelpContent() {
  ...
  const sections: { title: string; rows: [string, string][] }[] = [
    { title: t('summary.help_basic'), rows: [
    ...
    { title: t('summary.help_tables'), rows: [
      ['| A | B |\\n| --- | --- |\\n| 1 | 2 |', t('summary.help_table')],
    ]},
    { title: t('summary.help_math'), rows: [
      ['$E = mc^2$', t('summary.help_inline_math')],
      ['$$\\n\\\\sum_{i=1}^{n} x_i\\n$$', t('summary.help_block_math')],
    ]},
    { title: t('summary.help_footnotes'), rows: [
