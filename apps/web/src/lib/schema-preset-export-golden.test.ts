/**
 * Golden format contract for the standalone schema preset git export ZIP.
 *
 * Freezes the exact extracted-file tree `buildSchemaPresetZip` produces so the
 * server-side Python builder (workspace_export_assemble.build_schema_preset_tree)
 ...
 *   (or)     GOLDEN_UPDATE=1 npx vitest run src/lib/schema-preset-export-golden.test.ts
 */
...
import { buildSchemaPresetZip } from './entity-io'
import type { Storage } from '@/lib/storage'

const HERE = dirname(fileURLToPath(import.meta.url))
const GOLDEN_DIR = join(HERE, '__fixtures__', 'export-golden', 'schema-preset')
...
const storage = {
  schemaPresets: { getById: async () => input.preset },
...
async function buildTree(): Promise<Map<string, Uint8Array>> {
  const built = await buildSchemaPresetZip('preset-1', storage)
...
describe('Schema preset export — golden format contract', () => {
