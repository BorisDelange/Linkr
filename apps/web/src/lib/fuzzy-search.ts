...
/** Normalise a string column (or constant) for case- AND accent-insensitive
 *  comparison: `é → e`, `É → e`, `ç → c`, etc. DuckDB's `strip_accents` is
 *  Unicode-aware (NFD decompose + strip combining marks). */
...
export function buildFuzzySearchSql(term: string, cols: FuzzySearchColumns): FuzzySearchSql | null {
  ...
  const nameFolded = fold(`${prefix}${cols.nameColumn}`)
  const codeFolded = cols.codeColumn ? fold(`${prefix}${cols.codeColumn}`) : null
  const q = fold(`'${escaped}'`)
