/**
 * Golden format contract for the standalone DQ rule set git export ZIP.
 *
 * Freezes the exact extracted-file tree `buildDqRuleSetZip` produces so the
 * server-side Python builder (workspace_export_assemble.build_dq_rule_set_tree)
 ...
 *   (or)     GOLDEN_UPDATE=1 npx vitest run src/lib/dq-rule-set-export-golden.test.ts
 */
...
import { buildDqRuleSetZip } from './entity-io'
import type { Storage } from '@/lib/storage'

const HERE = dirname(fileURLToPath(import.meta.url))
const GOLDEN_DIR = join(HERE, '__fixtures__', 'export-golden', 'dq-rule-set')
...
async function buildTree(): Promise<Map<string, Uint8Array>> {
  const built = await buildDqRuleSetZip('rs-1', storage)
...
describe('DQ rule set export — golden format contract', () => {
