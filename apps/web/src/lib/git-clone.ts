 ...
 */
/**
 * Normalize a clone URL: drop a trailing slash and append `.git` when it's missing.
 * Many hosts (GitLab/framagit, GitHub) 301-redirect `…/repo` → `…/repo.git`; adding it
 * up front avoids the redirect entirely and works even on hosts that don't redirect.
 */
