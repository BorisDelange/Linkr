/**
 * Golden format contract for the standalone ETL pipeline git export ZIP.
 *
 * Freezes the exact extracted-file tree `buildEtlPipelineZip` produces so the
 * server-side Python builder (workspace_export_assemble.build_etl_pipeline_tree)
 ...
 *   (or)     GOLDEN_UPDATE=1 npx vitest run src/lib/etl-pipeline-export-golden.test.ts
 */
...
import { buildEtlPipelineZip } from './entity-io'
import type { Storage } from '@/lib/storage'

const HERE = dirname(fileURLToPath(import.meta.url))
const GOLDEN_DIR = join(HERE, '__fixtures__', 'export-golden', 'etl-pipeline')
...
async function buildTree(): Promise<Map<string, Uint8Array>> {
  const built = await buildEtlPipelineZip('pipe-1', storage)
...
describe('ETL pipeline export — golden format contract', () => {
