/**
 * Golden format contract for the standalone user plugin git export ZIP.
 *
 * Freezes the exact extracted-file tree `buildUserPluginZip` produces so the
 * server-side Python builder (workspace_export_assemble.build_user_plugin_tree)
 ...
 *   (or)     GOLDEN_UPDATE=1 npx vitest run src/lib/user-plugin-export-golden.test.ts
 */
...
import { buildUserPluginZip } from './entity-io'
import type { Storage } from '@/lib/storage'

const HERE = dirname(fileURLToPath(import.meta.url))
const GOLDEN_DIR = join(HERE, '__fixtures__', 'export-golden', 'user-plugin')
...
async function buildTree(): Promise<Map<string, Uint8Array>> {
  const built = await buildUserPluginZip('plug-1', storage)
...
describe('User plugin export — golden format contract', () => {
