/**
 * Golden format contract for the standalone SQL collection git export ZIP.
 *
 * Freezes the exact extracted-file tree `buildSqlCollectionZip` produces so the
 * server-side Python builder (workspace_export_assemble.build_sql_collection_tree)
 ...
 *   (or)     GOLDEN_UPDATE=1 npx vitest run src/lib/sql-collection-export-golden.test.ts
 */
...
import { buildSqlCollectionZip } from './entity-io'
import type { Storage } from '@/lib/storage'

const HERE = dirname(fileURLToPath(import.meta.url))
const GOLDEN_DIR = join(HERE, '__fixtures__', 'export-golden', 'sql-collection')
...
async function buildTree(): Promise<Map<string, Uint8Array>> {
  const built = await buildSqlCollectionZip('col-1', storage)
...
describe('SQL collection export — golden format contract', () => {
