...
/**
 * Build the pandas/scipy code that runs the statistical-test battery server-side
 * and prints the same TestResult[] JSON the client computes from rows. Statistics
 * (Welch t, Mann-Whitney U, chi², Fisher, ANOVA F, Kruskal-Wallis H, effect sizes,
 * CIs, descriptives) mirror computeAllTests exactly; distribution p-values use
 * scipy (at least as accurate as the browser's hand-rolled CDFs).
 */
export function buildStatisticalTestsCode(
  ...
  return `${_STAT_PY}\n_linkr_print_stats(dataset, _json.loads(${specStr}))\n`
}
