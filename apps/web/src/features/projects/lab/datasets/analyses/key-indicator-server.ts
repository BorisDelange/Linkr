...
/**
 * Build the pandas code that computes the Key Indicator server-side and prints a
 * KeyIndicatorData JSON matching what the component computes from rows front-only.
 * Only the small aggregate (scalar + stats + already-binned chart data) crosses
 * the wire — never the raw rows. Must stay in parity with the front-only compute
 * in KeyIndicatorComponent.tsx (numericResult / proportionResult / MiniChart).
 */
export function buildKeyIndicatorCode(columns: DatasetColumn[], config: Record<string, unknown>): string {
  ...
  return `${_KPI_PY}\n_linkr_print_kpi(dataset, _json.loads(${specStr}))\n`
}

const _KPI_PY = String.raw`
