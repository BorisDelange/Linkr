...
export function DiagnosisWidget() {
  ...
  const eventLabel = schemaMapping
    ? getDiagnosisEventLabel(schemaMapping)
    : null
  ...
  useEffect(() => {
    ...
    const sql = buildDiagnosesQuery(schemaMapping, patientId, visitId)
    if (!sql) {
      setData([])
      setLoading(false)
      return
    ...
    queryDataSource(dataSourceId, sql)
      .then((rows) => {
        if (!cancelled) setData((rows as DiagRow[]) ?? [])
      })
      .catch(() => {
    ...
    return (
      <div className="flex h-full items-center justify-center">
        <p className="text-xs text-muted-foreground">
          {t('patient_data.no_diagnosis_table')}
        </p>
      </div>
    )
  }
    ...
    <ScrollArea className="h-full">
      <div className="space-y-1">
        {data.map((row, i) => (
          ...
          >
            <Stethoscope size={10} className="shrink-0 text-red-500" />
