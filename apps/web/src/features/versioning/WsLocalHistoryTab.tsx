...
export function WsLocalHistoryTab() {
  ...
  const { commits, loading, fileChanges, createCommit } = useWorkspaceVersioningStore()
  ...
  const handleCommit = async () => {
    if (!wsUid || !canCommit) return
    await createCommit(wsUid, message.trim())
    setMessage('')
  }
  ...
  return (
    <>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-sm">{t('app_versioning.commit_title')}</CardTitle>
          <CardDescription>{t('app_versioning.commit_description')}</CardDescription>
        </CardHeader>
        <CardContent className="space-y-3">
          <Textarea
            value={message}
            onChange={(e) => setMessage(e.target.value)}
            placeholder={t('app_versioning.commit_placeholder')}
            rows={2}
          ...
          <div className="flex items-center justify-between">
            ...
            >
              ...
              {t('app_versioning.commit_button')}
            </Button>
          </div>
        </CardContent>
      </Card>
