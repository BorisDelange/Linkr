...
1. **Parse the dictionary — use the RESOLVED sets, not the raw expression.** A dictionary in this format has two parallel folders:
   - `concept_sets/<id>.json` holds the **OHDSI concept set *expression*** — a *definition*, not a target list. `expression.items[]` is a list of seed concepts each carrying boolean flags: `includeDescendants` (pull in every descendant via the vocabulary hierarchy), `includeMapped` (pull in every concept that "Maps to" this one), and `isExcluded` (remove this concept — and its descendants when combined with `includeDescendants`). The seeds are often high-level *classification* concepts (e.g. `37036048 "Sodium | Blood"`, `standardConcept:"C"`) with `includeDescendants:true`. Reading these `conceptId`s directly gives you the classification nodes, **not** the measurable standard targets — that is the trap.
   - `concept_sets_resolved/<id>.json` holds the **evaluated result** of that expression against the OMOP vocabulary: the concrete standard `conceptId`s the set actually targets (e.g. the canonical `3019550 Sodium [Moles/volume] in Serum or Plasma`), under `resolvedConcepts[]` (each entry has `conceptId`, `conceptName`, `vocabularyId`, `standardConcept`).

   **Always build `dict_targets` from `concept_sets_resolved/`** (keep `standardConcept:"S"`), and read the metadata (`metadata.uniqueId`, `metadata.sourceRepo`, `metadata.translations.<lang>.category`/`subcategory`, set name) from the matching `concept_sets/<id>.json`. Join on the shared filename / `conceptSetId`. Build `dict_targets(concept_id, concept_set_uid, source_repo, category, subcategory, set_name)`. **Note: the same `concept_id` can belong to several sets** — keep all `(concept_id → set)` links.
2. **Read each set's `longDescription` — it is the mapping authority (MANDATORY when present).** Each `concept_sets/<id>.json` carries `metadata.translations.<lang>.longDescription`, a Markdown text with (when present) sections `## Definition & Clinical Context`, `## Included Concepts`, `## Excluded Concepts`, and — most importantly — **`## Mapping Notes`**. The Mapping Notes name the **default target concept** for the set ("when the source records X without further specification, map to *conceptId / code — name*"), give **conditional rules** (e.g. blood-gas analyser → arterial specimen; pulse-oximetry HR → the by-oximetry LOINC; HFNC → the specific high-flow concept), and list **do-not-confuse / out-of-scope** traps. In this dictionary, ~187/366 sets have a longDescription and ~157 have explicit Mapping Notes. **Before choosing a target for any source concept, you MUST read the stamped set's `longDescription` and follow its Mapping Notes**: pick the named default concept unless the source's own `metadata_json` documents a specificity (site, method, specimen, setting-vs-measured) that the notes map to a named variant; never map to a concept the notes list as excluded/out-of-scope. The longDescription **overrides the biolord/jaro-winkler ranking** — a high similarity score to an excluded or wrong-specimen concept does not justify choosing it. When a set has no Mapping Notes, fall back to clinical judgement over `resolvedConcepts`.
3. **Category by category** — process one `category` at a time (the user picks the order; default: whatever they name first). This keeps each review clinically coherent and reviewable.
4. **Invert the mapping direction** — instead of "for each source, find a target", the natural framing is "for each dictionary target in this category, which source concepts align onto it?". Use the pre-computed `similarity-scores.parquet` (biolord + jaro-winkler) restricted to `concept_id ∈ dict_targets`, keep source candidates above a similarity threshold (default `semantic/biolord ≥ 0.5`), and hand those candidate pairs to `/concept-mapping-ai`.
5. **Priority + OMOP fallback** — the AI's target-selection order is: **dictionary candidate (guided by the set's Mapping Notes) → dictionary by direct search** (a low similarity score can hide a correct match for a terse source label like `VT`, `PEP`, `FR` — search the dictionary targets by name/synonym) **→ full OMOP** (search the whole `concept` table, standard+valid) **→ `no-match`**. When the chosen target is a dictionary concept, the AI stamps the suggestion with that set's `concept_set_uid` + `source_repo`; when it falls back to a plain OMOP target, both are left null and the `comment` notes it is outside the dictionary. If the set's Mapping Notes mark the only plausible target as out-of-scope, prefer the full-OMOP fallback (uid null) over forcing an excluded concept.
